...
import threading
import time
from coinmetrics.utils.event import EventEmitter
...
def reportPipelineExceptions(log, pipelines, tasks, taskRepr):
    for pipeline in pipelines:
        if pipeline.faulted():
            message = []
            for taskIndex, exception in pipeline.exceptions:
                if taskRepr is None:
                    message.append("task index %d:\n" % taskIndex)
                else:
                    message.append("task %s:\n" % taskRepr(tasks[taskIndex]))
                message.append("%s" % exception)
            log.critical("exceptions during execution of pipeline %s:\n%s" % (pipeline, "".join(message)))


def runPipelineChain(tasks, pipelines, log, eta=None, taskRepr=None, prefetchCount=1):
    ...
    for pipeline in pipelines:
        log.info("shutting down pipeline %s" % pipeline)
        pipeline.shutdown()

    reportPipelineExceptions(log, pipelines, tasks, taskRepr)
