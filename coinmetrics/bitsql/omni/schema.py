...
class OmniSchema(object):
    ...
    def init(self):
        self.blocksTableName = "omni_blocks"
        ...
        self.simpleSendTxTableName = "omni_simple_send_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            simple_send_tx_hash DECIMAL(%s) PRIMARY KEY, \
            simple_send_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.simpleSendTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH,
                  MAX_ADDRESS_LENGTH, OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.sendAllTxTableName = "omni_send_all_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            send_all_tx_hash DECIMAL(%s), \
            send_all_tx_index INTEGER, \
            send_all_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.sendAllTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                MAX_ADDRESS_LENGTH, OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.sendOwnersTxTableName = "omni_send_owners_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            send_owners_tx_hash DECIMAL(%s) PRIMARY KEY, \
            send_owners_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.sendOwnersTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))
        
        self.sellForBitcoinTxTableName = "omni_sell_for_bitcoin_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            sell_for_bitcoin_tx_hash DECIMAL(%s) PRIMARY KEY, \
            sell_for_bitcoin_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.sellForBitcoinTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.acceptSellForBitcoinTxTableName = "omni_accept_sell_for_bitcoin_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            accept_sell_for_bitcoin_tx_hash DECIMAL(%s) PRIMARY KEY, \
            accept_sell_for_bitcoin_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.acceptSellForBitcoinTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION, MAX_ADDRESS_LENGTH))

        self.dexPurchaseTxTableName = "omni_dex_purchase_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            dex_purchase_tx_hash DECIMAL(%s), \
            dex_purchase_tx_index INTEGER, \
            dex_purchase_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.dexPurchaseTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION, MAX_ADDRESS_LENGTH))

        self.createFixedPropertyTxTableName = "omni_create_fixed_property_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            create_fixed_property_tx_hash DECIMAL(%s) PRIMARY KEY, \
            create_fixed_property_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.createFixedPropertyTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.createManagedPropertyTxTableName = "omni_create_managed_property_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            create_managed_property_tx_hash DECIMAL(%s) PRIMARY KEY, \
            create_managed_property_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.createManagedPropertyTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION))

        self.grantTokensTxTableName = "omni_grant_tokens_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            grant_tokens_tx_hash DECIMAL(%s) PRIMARY KEY, \
            grant_tokens_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.grantTokensTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                MAX_ADDRESS_LENGTH, OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.revokeTokensTxTableName = "omni_revoke_tokens_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            revoke_tokens_tx_hash DECIMAL(%s) PRIMARY KEY, \
            revoke_tokens_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.revokeTokensTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.createCrowdsalePropertyTxTableName = "omni_create_crowdsale_property_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            create_crowdsale_property_tx_hash DECIMAL(%s) PRIMARY KEY, \
            create_crowdsale_property_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.createCrowdsalePropertyTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION, OUTPUT_VALUE_PRECISION))

        self.closeCrowdsaleTxTableName = "omni_close_crowdsale_transactions"
        self.dbAccess.queryNoReturnCommit("CREATE TABLE IF NOT EXISTS %s (\
            close_crowdsale_tx_hash DECIMAL(%s) PRIMARY KEY, \
            close_crowdsale_tx_block_hash DECIMAL(%s), \
            ...
            )" % (self.closeCrowdsaleTxTableName, HASH_PRECISION, HASH_PRECISION, MAX_ADDRESS_LENGTH, 
                OUTPUT_VALUE_PRECISION))
    ...
    def addIndexes(self):
        self.dbAccess.queryNoReturnCommit("CREATE INDEX omni_simple_send_transactions_time_index ON %s(simple_send_tx_time)" % self.simpleSendTxTableName)
        self.dbAccess.queryNoReturnCommit("CREATE INDEX omni_send_owners_transactions_time_index ON %s(send_owners_tx_time)" % self.sendOwnersTxTableName)
        self.dbAccess.queryNoReturnCommit("CREATE INDEX omni_grant_tokens_transactions_time_index ON %s(grant_tokens_tx_time)" % self.grantTokensTxTableName)
        self.dbAccess.queryNoReturnCommit("CREATE INDEX omni_revoke_tokens_transactions_time_index ON %s(revoke_tokens_tx_time)" % self.revokeTokensTxTableName)
        self.dbAccess.queryNoReturnCommit("CREATE INDEX omni_send_all_transactions_time_index ON %s(send_all_tx_time)" % self.sendAllTxTableName)

    def dropIndexes(self):
        self.dbAccess.queryNoReturnCommit("DROP INDEX omni_send_all_transactions_time_index")
        self.dbAccess.queryNoReturnCommit("DROP INDEX omni_revoke_tokens_transactions_time_index")
        self.dbAccess.queryNoReturnCommit("DROP INDEX omni_grant_tokens_transactions_time_index")
        self.dbAccess.queryNoReturnCommit("DROP INDEX omni_send_owners_transactions_time_index")
        self.dbAccess.queryNoReturnCommit("DROP INDEX omni_simple_send_transactions_time_index")
