...
from coinmetrics.bitsql.data import *
...
class BitcoinNodeBase(object):
    ...
    def __repr__(self):
        return self.bitcoinAccess.getAddress()

    def getBlockCount(self):
        return self.bitcoinAccess.call("getblockcount")

    def supportsGetBlock2(self):
        return True

    def getBlock(self, height):
        blockHash = self.bitcoinAccess.call("getblockhash", [height])
        if self.supportsGetBlock2():
            blockDict = self.bitcoinAccess.call("getblock", [blockHash, 2])
        else:
    ...
    def getBlockTransactions(self, blockDict, blockData):
        if self.supportsGetBlock2():
            txDicts = blockDict["tx"]
            result = []
            for tx in txDicts:
                if not self.excludeTransaction(tx["txid"], blockData):
                    result.append(tx)
            return result
        else:
            hashes = []
    ...
    def initBlock(self, blockDict):
        ...
        medianTime = self.getBlockMedianTime(blockDict)
        blockSize = int(blockDict["size"])
        difficulty = float(blockDict["difficulty"])
        height = int(blockDict["height"])
        return BitcoinBlockData(height, hashAsNumber, chainWorkAsNumber, blockTime, medianTime, blockSize, difficulty)

    def getBlockChainWork(self, blockDict):
        return int(blockDict["chainwork"], base=16)

    def getBlockMedianTime(self, blockDict):
        return datetime.utcfromtimestamp(blockDict["mediantime"])

    def processTransaction(self, txDict, txIndex, blockData):
        transaction = self.initTransaction(txDict, txIndex, blockData)
        self.processInputs(transaction, txDict, txIndex, blockData)
        self.processOutputs(transaction, txDict, txIndex)
        if self.transactionIsCoinbase(txDict, txIndex, blockData) and self.shouldSaveCoinbaseScript(txDict, txIndex, blockData):
            assert len(txDict["vin"]) == 1
            transaction.setCoinbaseScript(txDict["vin"][0]["coinbase"])
        return transaction
    ...
    def initTransaction(self, txDict, txIndex, blockData):
        ...
        return BitcoinTransactionData(txHashAsNumber, txSize, blockData.blockTime, blockData.blockMedianTime, coinbase)
    ...
    def shouldSaveCoinbaseScript(self, txDict, txIndex, blockData):
        return True

    def processInputs(self, transaction, txDict, txIndex, blockData):
    ...
    def processInput(self, transaction, inputDict, inputIndex):
        ...
        scriptSig = inputDict["scriptSig"]["hex"]
        transaction.addInput((inputTxHashAsNumber, outputIndex, scriptSig))
    ...
    def processOutput(self, transaction, outputDict, outputIndex):
        ...
        if outputType in [OUTPUT_TYPES["nonstandard"], OUTPUT_TYPES["nulldata"], OUTPUT_TYPES["multisig"]]:
            ...
            self.processSegwitOutput(outputDict, addresses)
        else:
            assert len(addresses) == 1
...
class BitcoinGoldNode(BitcoinNode):
    ...
    def processSegwitOutput(self, outputDict, addresses):
        ...
        if len(addresses) == 0:
            ...
            addresses.append(bech32.encode(prefix, 0, binascii.unhexlify(outputDict["scriptPubKey"]["hex"][4:])))


class BitcoinSvNode(BitcoinNode):
...
class ZcashNode(BitcoinNodeBase):
    ...
    def getBlockMedianTime(self, blockDict):
        # ZCash node doesn't report median block time
        return datetime(1970, 1, 1)

    def initTransaction(self, txDict, txIndex, blockData):
        data = super(ZcashNode, self).initTransaction(txDict, txIndex, blockData)
        return ZcashTransactionData(data.txHash, data.txSize, data.txTime, data.txMedianTime, data.coinbase)
...
class BitcoinPrivateNode(ZcashNode):
...
class DogecoinNode(BitcoinNodeBase):
    ...
    def getBlockMedianTime(self, blockDict):
        # dogecoin node doesn't report median block time
        return datetime(1970, 1, 1)
...
class PivxNode(BitcoinNode):
    ...
    def getBlockMedianTime(self, blockDict):
        # PIVX node doesn't report median block time
        return datetime(1970, 1, 1)

    def initTransaction(self, txDict, txIndex, blockData):
        data = super(PivxNode, self).initTransaction(txDict, txIndex, blockData)
        return PivxTransactionData(data.txHash, data.txSize, data.txTime, data.txMedianTime, data.coinbase)
...
class DecredNode(BitcoinNodeBase):
    ...
    def excludeTransaction(self, txHash, blockData):
        return (txHash, blockData.blockHeight) in self.duplicateTransactions or txHash in self.missingInfoTransactions
    ...
    def initTransaction(self, txDict, txIndex, blockData):
        ...
        return DecredTransactionData(data.txHash, data.txSize, data.txTime, data.txMedianTime, data.coinbase)
...
class DashNode(BitcoinNodeBase):
